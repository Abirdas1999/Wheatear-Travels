...
<nav class="navbar navbar-expand-md navbar-dark navbar_main">
    ...
    <div class="container py-5 my-5">
      <h3 class="h3_new py-5">
        What are the advantages of online bus booking?
      </h3>
    
      
      <div class="faqs-container">  
        <details open name="faq">
          <summary style="font-size: 1.34rem;">
            What are the advantages of online bus booking?
          </summary>
          <?php
            $query = "SELECT * FROM bus WHERE id = 2";
          ...
          <?php echo $row['question']; // Debugging: Show the file path ?>
            ...
            ?>
            <?php
            $query = "SELECT * FROM bus WHERE id = 3";
            $query_run  = mysqli_query($conn, $query);
            if ($query_run && mysqli_num_rows($query_run) > 0) {
            foreach ($query_run as $row) {
          ?>
          ...
          <?php echo $row['question']; // Debugging: Show the file path ?>
            ...
            ?>
            <?php
            $query = "SELECT * FROM bus WHERE id = 4";
            $query_run  = mysqli_query($conn, $query);
            if ($query_run && mysqli_num_rows($query_run) > 0) {
            foreach ($query_run as $row) {
          ?>
          ...
          <?php echo $row['question']; // Debugging: Show the file path ?>
            ...
            ?>
            <?php
            $query = "SELECT * FROM bus WHERE id = 5";
            $query_run  = mysqli_query($conn, $query);
            if ($query_run && mysqli_num_rows($query_run) > 0) {
            foreach ($query_run as $row) {
          ?>
          ...
          <?php echo $row['question']; // Debugging: Show the file path ?>
            ...
            ?>
            <?php
            $query = "SELECT * FROM bus WHERE id = 7";
            $query_run  = mysqli_query($conn, $query);
            if ($query_run && mysqli_num_rows($query_run) > 0) {
            foreach ($query_run as $row) {
          ?>
